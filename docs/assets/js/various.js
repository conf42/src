function getCountry() {
	...
	window.countryNames = countries; // exposed for countryCodeForName() (dial-code prefill)
	var timezones = {
		"Africa/Abidjan": {
			u: 0,
...
// International dialling codes by ISO-3166 alpha-2 country code (used to prefill the phone field)
var dialCodes = {
  AD:"+376",AE:"+971",AF:"+93",AQ:"+672",BV:"+47",GS:"+500",HM:"+672",PN:"+64",TF:"+262",AG:"+1",AI:"+1",AL:"+355",AM:"+374",AO:"+244",AR:"+54",AS:"+1",AT:"+43",AU:"+61",AW:"+297",AX:"+358",AZ:"+994",
  ...
  PA:"+507",PE:"+51",PF:"+689",PG:"+675",PH:"+63",PK:"+92",PL:"+48",PM:"+508",PR:"+1",PS:"+970",PT:"+351",PW:"+680",PY:"+595",QA:"+974",
  ...
  UA:"+380",UG:"+256",UM:"+1",US:"+1",UY:"+598",UZ:"+998",VA:"+39",VC:"+1",VE:"+58",VG:"+1",VI:"+1",VN:"+84",VU:"+678",WF:"+681",WS:"+685",XK:"+383",YE:"+967",YT:"+262",ZA:"+27",ZM:"+260",ZW:"+263"
};

// Select option names that differ from the `countries` map (or would loosely match the wrong entry)
var countryAliases = {
  "aland islands":"AX","bonaire, sint eustatius and saba":"BQ","cape verde":"CV","cocos (keeling) islands":"CC",
  "cote d'ivoire":"CI","curacao":"CW","czech republic":"CZ","korea, democratic people's republic of":"KP",
  "korea, republic of":"KR","kosovo":"XK","lao people's democratic republic":"LA",
  "macedonia, the former yugoslav republic of":"MK","palestinian territory, occupied":"PS","reunion":"RE",
  "saint barthelemy":"BL","swaziland":"SZ","viet nam":"VN","virgin islands, british":"VG","virgin islands, u.s.":"VI",
  "congo":"CG","taiwan, province of china":"TW","netherlands antilles":"CW","serbia and montenegro":"RS"
};

// ISO code for a country name as it appears in the form's select (matched against the `countries` map)
function countryCodeForName(name) {
    if (!name) return null;
    var a = name.toLowerCase();
    if (countryAliases[a]) return countryAliases[a];
    if (!window.countryNames) { try { getCountry(); } catch (e) {} }
    var names = window.countryNames || {};
    var code;
    for (code in names) {                       // exact name first ("United States" vs "United States Minor Outlying Islands")
        if (String(names[code]).toLowerCase() === a) return code;
    }
    for (code in names) {                       // then a loose match ("Russian Federation" vs "Russia")
        var b = String(names[code]).toLowerCase();
        if (a.includes(b) || b.includes(a)) return code;
    }
    return null;
}

// Prefill the phone field with the dialling code of the selected (or browser-detected) country.
...
function suggestDialCode() {
    ...
    if (select && select.value) {
        code = countryCodeForName(select.value);      // a chosen country wins...
    } else {
        try { var c = getCountry(); code = c && c.code; } catch (e) { code = null; }   // ...else the browser's location
    }
